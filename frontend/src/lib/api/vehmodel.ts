...
import { VehModelPublic, VehModelsPublic, VehModelCreate, VehModelUpdate } from "@/types/vehmodel_types"
...
export const vehmodelApi = {
    getAll: async (skip = 0, limit = 100): Promise<VehModelsPublic> => {
        ...
        if (!response.ok) throw new Error(`Vehicle Model GET Error: ${response.status}`);
